...
<html>
    <head>
        <meta charset="UTF-8">
        <title>Decoration Bookings - Seller View</title>
        ...
        <nav class="navbar">
            <div class="nav-container">
                ...
                <div class="nav-links">
                    ...
                    <a href="decoration_seller.jsp" class="nav-link">Seller Page</a>
                    <% if (role == null) { %>
        ...
        <div class="form-container">
            ...
            <table>
                ...
                <%
                    try {
                        PreparedStatement stmt = conn.prepareStatement(
                            "SELECT b.*, d.owner FROM decoration_bookings b INNER JOIN decoration_services d ON b.decoration_id = d.id WHERE d.owner = ? ORDER BY b.created_at DESC"
                        );
                ...
                <tr>
                    ...
                    <td><%= rs.getString("decoration_name") %></td>
                    <td><%= status.substring(0, 1).toUpperCase() + status.substring(1) %></td>
                    <td>
                        <% if ("pending".equals(status)) { %>
                        <a href="update_decoration_booking.jsp?id=<%= rs.getInt("id") %>&status=confirmed" class="btn btn-confirm">Confirm</a>
                        <a href="update_decoration_booking.jsp?id=<%= rs.getInt("id") %>&status=rejected" class="btn btn-reject">Reject</a>
                        <% } else { %>
            ...
            <div style="text-align:center; margin-top: 20px;">
                <a href="decoration_seller.jsp">← Back to Seller Page</a>
            </div>
        </div>
        <script>
            // Toggle dropdown
